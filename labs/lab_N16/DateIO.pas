...
TYPE
  Month = (NoMonth, Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec); {Месяцы}
PROCEDURE ReadMonth(VAR FIn: TEXT; VAR Mo: Month); {Чтение Mo из FIn}
PROCEDURE WriteMonth(VAR FOut: TEXT; VAR Mo: Month); {Вывод Mo в FOut}

IMPLEMENTATION

PROCEDURE ReadMonth(VAR FIn: TEXT; VAR Mo: Month); {Читает входную строку и распознает месяц}
VAR
  Ch1, Ch2, Ch3: CHAR;
BEGIN {ReadMonth}
...
PROCEDURE WriteMonth(VAR FOut: TEXT; VAR Mo: Month); {Выводит поточное представление месяца символами}
BEGIN {WriteMonth}
